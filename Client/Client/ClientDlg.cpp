...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_USN, m_username);
...
void CClientDlg::OnPaint()
{
	...
	else
	{
		// Title
		CFont font;
		font.CreateFontW(50, 0, 0, 0, FW_BOLD, 0, 0, 0, 0, 0, 0, 0, 0, L"Arial");
		CFont* pFont = dc.SelectObject(&font);
		dc.SetTextColor(RGB(255, 62, 76));
		dc.SetBkMode(TRANSPARENT);
		dc.SetTextAlign(TA_CENTER);
		dc.TextOut(380, 40, L"EMAIL SECURITY");
...
void CClientDlg::OnBnClickedLogin()
{
	...
	username = text; //set username for main dialog
	send(client, CT2A(text), len, 0);
	recvMessage = recv(client, buffer, BUF_SIZE, 0);
	buffer[recvMessage] = '\0';
	Sleep(10);
	m_password.GetWindowTextW(text);
	len = m_password.GetWindowTextLengthW();
	send(client, CT2A(text), len, 0);
	recvMessage = recv(client, buffer, BUF_SIZE, 0);
	buffer[recvMessage] = '\0';
